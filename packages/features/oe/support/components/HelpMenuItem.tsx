...
export default function HelpMenuItem({ onHelpItemSelect }: HelpMenuItemProps) {
  const [selectedScore, setSelectedScore] = useState<null | string>(null);
  const [userFeedback, setUserFeedback] = useState("");
  const [isSubmissionBlocked, setIsSubmissionBlocked] = useState(true);
  const { t } = useLocale();

  const feedbackMutation = trpc.viewer.submitFeedback.useMutation({
    onSuccess: () => {
  ...
  const handleScoreSelection = (scoreValue: string) => {
    setSelectedScore(scoreValue);
    setIsSubmissionBlocked(false);
  };

  const processFeedbackSubmission = async (score: string, feedback: string) => {
    feedbackMutation.mutate({ rating: score, comment: feedback });
  };

  const renderEmoticonButton = (emotion: string, svgContent: JSX.Element) => (
    <button
      className={classNames(
        "m-1 items-center justify-center p-1.5 grayscale hover:opacity-100 hover:grayscale-0",
        selectedScore === emotion ? "grayscale-0" : "opacity-50"
      )}
      onClick={() => handleScoreSelection(emotion)}>
      {svgContent}
    </button>
  );

  const ExtremelyUnsatisfiedIcon = (
  ...
  const SupportContactSection = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className={`text-subtle bg-muted w-full p-5 ${isMobile ? "md:hidden" : "hidden md:block"}`}>
      <p>{t("specific_issue")}</p>
      <a href="mailto:support@onehash.ai">
        <button className="hover:text-emphasis text-defualt font-medium underline">
          {t("contact_support")}
        </button>
      </a>
    ...
    <div className="bg-default border-default w-full rounded-md">
      ...
      <div className="w-full p-5">
        ...
        <div className="my-3 flex justify-end">
          {renderEmoticonButton("Extremely unsatisfied", ExtremelyUnsatisfiedIcon)}
          {renderEmoticonButton("Unsatisfied", UnsatisfiedIcon)}
          {renderEmoticonButton("Satisfied", SatisfiedIcon)}
          {renderEmoticonButton("Extremely satisfied", HighlySatisfiedIcon)}
        </div>
        ...
        <div className="my-2 flex justify-end">
          <Button
            ...
            onClick={async () => {
              if (selectedScore && userFeedback) {
                await processFeedbackSubmission(selectedScore, userFeedback);
              }
            }}>
            {t("submit")}
          </Button>
      ...
      <SupportContactSection isMobile={true} />
    </div>
  );
}
