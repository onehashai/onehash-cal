...
const HelpPanel = ({ onHelpItemSelect }: HelpPanelProps) => {
  ...
  const handleRatingSelection = (ratingValue: string) => {
    setSelectedRating(ratingValue);
    setIsSubmitDisabled(false);
  };

  const submitUserFeedback = async (userRating: string, userComment: string) => {
    feedbackMutation.mutate({ rating: userRating, comment: userComment });
  };

  const renderEmojiButton = (emojiRating: string, svgContent: JSX.Element) => (
    <button
      className={classNames(
        "m-1 items-center justify-center p-1.5 grayscale hover:opacity-100 hover:grayscale-0",
        selectedRating === emojiRating ? "grayscale-0" : "opacity-50"
      )}
      onClick={() => handleRatingSelection(emojiRating)}>
      {svgContent}
    </button>
  );

  const extremelyUnsatisfiedEmoji = (
  ...
  const renderSupportSection = (isDesktop: boolean) => (
    <div
      className={classNames("text-subtle bg-muted w-full p-5", isDesktop ? "hidden md:block" : "md:hidden")}>
    ...
    <div className="bg-default border-default w-full rounded-md">
      ...
      <div className="w-full p-5">
        ...
        <div className="my-3 flex justify-end">
          {renderEmojiButton("Extremely unsatisfied", extremelyUnsatisfiedEmoji)}
          {renderEmojiButton("Unsatisfied", unsatisfiedEmoji)}
          {renderEmojiButton("Satisfied", satisfiedEmoji)}
          {renderEmojiButton("Extremely satisfied", extremelySatisfiedEmoji)}
        </div>
        <div className="my-2 flex justify-end">
          <Button
            ...
            onClick={async () => {
              if (selectedRating && userComment) {
                await submitUserFeedback(selectedRating, userComment);
              }
            }}>
            {t("submit")}
          </Button>
      ...
      {renderSupportSection(true)}
      {renderSupportSection(false)}
    </div>
  );
};
