      ...
      try {
        ...
        if (notification.workflowStep.reminderBody) {
          ...
          messageContent = emailReminderTemplate(
            false,
            notification.booking.user?.locale || "en",
            notification.workflowStep.action,
            getTimeFormatStringFromUserTimeFormat(notification.booking.user?.timeFormat),
            notification.booking.startTime.toISOString() || "",
            notification.booking.endTime.toISOString() || "",
            notification.booking.eventType?.title || "",
            userTimeZone || "",
            notification.booking.location || "",
            bookingMetadataSchema.parse(notification.booking.metadata || {})?.videoCallUrl || "",
            participantName || "",
            recipientName || "",
            !!notification.booking.user?.hideBranding
          );
...
const handler = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  ...
  res.status(200).json({ message: `${pendingNotifications.length} Emails to schedule` });
};
