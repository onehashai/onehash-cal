...
2. Add an OAuth client.
   ...
   2. Redirect URIs: Used to validate internal requests origin and allow redirections on your platform when needed. You can enable any URI of your company’s domain (e.g. cal.com in our case).
...
function MyApp({ Component, pageProps }) {
  ...
  useEffect(() => {
    fetch(`/api/users/${pageProps.userId}`, {}).then(async (res) => {
